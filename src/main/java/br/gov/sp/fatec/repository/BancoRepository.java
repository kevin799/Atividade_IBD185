...
public interface BancoRepository extends CrudRepository<Banco,Long>{
	...
	@Query(value="select c* from ban_banco b JOIN ser_servidor s ON (b.ser_id = s.ser_id) where s.ser_ip = :ip", nativeQuery = true)
	public List<Banco> findByAll(@Param("ip") String ip );
